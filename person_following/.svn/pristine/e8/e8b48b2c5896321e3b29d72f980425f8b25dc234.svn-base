#include <PersonFollowerRobocup.h>


PersonFollower::PersonFollower(): nh_("~"),ac("move_base", true),listener3(ros::Duration(10)),follow_ac("followMe_action"),wait_ac("wait_action"),stop_ac("stop_action")
{
	using namespace std;
	...
	maxEuclidianDistOfSegmentsThatCanBePersonCandids=0.5; //0.6 before
	minEuclidianDistOfSegmentsThatCanBePersonCandids=0.03; //0.1 before
	...
	tetaNearby=CV_PI/2;
	velocityCounter=0;
	maxDistanceToMatchPrediction=0.4;
 ...
 cout<<"logLaser: "<<logLaser<<endl;
	if(useVoice)
	{
        sc.say("Initialized Follow Me Task Waiting for the Follow Me command");
	}
	cvWaitKey(7.0);
...
*/
	...
	else //(virgin==false)
	{
		...
		else if(currentState==ONE_LEG_TRACK_MODE)
		{
			...
			//cout<<"BEST MATCH : "<<minDistanceToPredictedState<<endl;
...
/*
					if (nh_.hasParam("/PersonRecognizerRobocup/searchedPersonsName"))
					{
						nh_.setParam("/PersonRecognizerRobocup/searchedPersonsName", "SEARCH_NOONE");
					}
					if (nh_.hasParam("/PersonRecognizerRobocup/recognitionThreshold"))
					{
						double temprecognitionThreshold;
						nh_.setParam("/PersonRecognizerRobocup/recognitionThreshold",defaultStartingRecognitionThreshold);
					}
*/
				...
				else if(maxDistanceToMatchPrediction>0.5)
				{
					maxDistanceToMatchPrediction=0.5;
				}
