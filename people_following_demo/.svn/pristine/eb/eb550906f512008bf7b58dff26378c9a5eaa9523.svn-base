...
const double extraVelocityBeCareful=0.04;
...
{	 
	...
	laser_point.header.frame_id = "base_laser"; //tf
				...
				else //there is a moved segment
				{
					...
					double minMovingSegmentDistance=100.0;					
						...
						if(abs(centerPtt[0])<rectangularBoxXover2sizeForMovDetection && abs(centerPtt[1])<rectangularBoxYsizeForMovDetection)
						{
												...
												//TODO: transform valsXK
												laser_point.header.stamp = ros::Time();
												laser_point.point.x = valsXk[0];
												laser_point.point.y = valsXk[1];
												try
												  {
												    ...
												    valsXk[0]=base_point.point.x;
												    valsXk[1]=base_point.point.y;
												  }
									...
									if(closestSegmentToPredictedState!=100 && minDistanceToPredictedState<maxDistanceToMatchPrediction)
						   			   {
											...
											laser_point.header.stamp = ros::Time();
											laser_point.point.x = checkedCenterPoint[0];
											laser_point.point.y = checkedCenterPoint[1];
											try
											  {
											    geometry_msgs::PointStamped base_point;
											    listener.transformPoint("base_link", laser_point, base_point);
											    ...
											    checkedCenterPoint[0]=base_point.point.x;
											    checkedCenterPoint[1]=base_point.point.y;
											  }
											  catch(tf::TransformException& ex){
											    ROS_ERROR("Received an exception trying to transform a point from \"base_laser\" to \"base_link\": %s", ex.what());
											  }
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber chatter_sub=n.subscribe("scan",3,scanCallback);
