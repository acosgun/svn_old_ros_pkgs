...
PersonRecognizer::PersonRecognizer(): nh_("~")
{
    ...
    //scoringBiasTowardsFaceRecog=0.5; //Scoring=(face recognition Score)*scoringBiasTowardsFaceRecog+(shirt color score)*(1-scoringBiasTowardsFaceRecog)
    ...
    //THRESHOLD_FOR_PERSON_RECOGNITION=0.65; //between [0-1].to be recognized as a person, the max Scorer must surpass this score. for 0 every face, for 1 none is accepted. 0.6 ideal?
	...
	{
		cout<<"ERROR: Could not get parameter for cascade file"<<endl;
		exit(0);
	}
...
void PersonRecognizer::commandCallback  (const std_msgs::String::ConstPtr& msg)
{
	...
	case 'r': //enable recognizer
		...
		if(msg->data.length()>2)
		{
			...
			case 'q':
				nh_.deleteParam("recognitionThreshold");
				nh_.deleteParam("searchedPersonsName");
				nh_.deleteParam("currentState");
				nh_.deleteParam("isNameNeeded");
				nh_.deleteParam("learnedPersonsName");
				exit(0);
				break;
			default:
				break;
...
//bool PersonRecognizer::ReceiveImage(const sensor_msgs::ImageConstPtr& img, const sensor_msgs::CameraInfoConstPtr& cam_info)
//{
...
void PersonRecognizer::ImageCallback(const sensor_msgs::ImageConstPtr& msg) 
{
	...
	int temp;
	if(nh_.getParam("currentState", temp))
	{
	setState((State)temp);
	}
...
//	    }
	...
	else if(currentState==LEARN_PERSON)
	{
		...
		if(learnStateVirgin==true)
		{
			...
			if(nh_.getParam("isNameNeeded", isNameNeeded)) // update the parameter.
			{ }
			if(isNameNeeded==true)
			{
				if(nh_.getParam("learnedPersonsName", learnedPersonsName)) // update the parameter.
				{ }
				currentNumberOfFaceImagesSaved=0;
	    ...
	    findFaces(global_image, CV_HAAR_FIND_BIGGEST_OBJECT,faceDetectionQualityWhenLearning,faceRectanglesVector); //looking for the biggest face in the image.
	...
	else if(currentState==RECOGNIZE_PEOPLE_FROM_FACES)
	{
		...
		if(nh_.getParam("recognitionThreshold",THRESHOLD_FOR_PERSON_RECOGNITION)) //update parameters
		{

		}
		if(nh_.getParam("searchedPersonsName", searchedPersonsName))
		{

		}

	    vector<CvRect> faceRectanglesVector;
	    findFaces(global_image, CV_HAAR_DO_CANNY_PRUNING, faceDetectionQualityWhenRecognition ,faceRectanglesVector); // should be CV_HAAR_DO_CANNY_PRUNING. for 1 face=CV_HAAR_FIND_BIGGEST_OBJECT
...
//							    }
			 ...
			 if ((maxScore>THRESHOLD_FOR_PERSON_RECOGNITION && ((maxScore/SecondMaxScore)>THRESHOLD_FOR_RATIO_TEST))) //this tested face rectange faceRectanglesVector[h] with torso rectange torsoRect matches the person personList[j]
			 {
				 ...
				 if(personList[maxScoreIndex].shirtHistogramInitialized==true) // UPDATE THE HISTOGRAM
				 {
...
void PersonRecognizer::setState(State newState)
{
	...
	nh_.setParam("currentState", currentState);
}
...
void PersonRecognizer::setSearchedPersonsName(string name)
{
	...
	nh_.setParam("searchedPersonsName", searchedPersonsName);
}
...
void PersonRecognizer::findFaces(IplImage* img, int flags, int min_neighbors,vector<CvRect>& faceRectanglesVector)
{
    ...
    cout<<"Cascade not loaded properly for face detection"<<endl;
        ...
        CvSeq* faces = cvHaarDetectObjects( gray_img, cascade, storage,1.2, min_neighbors, flags,cvSize(30, 30)); //FOR TRAINING 70 pixels=detect max 2m.
...
void PersonRecognizer::increaseRecognitionThreshold(double addedValue)
{
	...
	nh_.setParam("recognitionThreshold",THRESHOLD_FOR_PERSON_RECOGNITION);
}
